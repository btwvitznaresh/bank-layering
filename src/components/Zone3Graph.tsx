...
export const Zone3Graph: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const runSequence = async () => {
      ...
      cy.fit(undefined, 50);
      addToast("Scanning network...", "info");
      ...
      addToast("⚠ High-value node detected: ACC012 appears in 4 cases", "warning");
      await new Promise(r => setTimeout(r, 2000));
      if (isCancelled) return;

      cy.edges('.suspicious-edge').addClass('search-flash');
      addToast("⚠ Circular transaction loop: ACC003 → ACC008 → ACC015", "error");
      await new Promise(r => setTimeout(r, 2000));
      ...
      addToast("⚠ VPN activity detected: 185.220.101.45", "warning");
      await new Promise(r => setTimeout(r, 2000));
      if (isCancelled) return;

      cy.edges('.inter-case').addClass('search-flash');
      addToast("⚠ Inter-case money flow: ₹1.24Cr via ACC007", "warning");
      await new Promise(r => setTimeout(r, 2000));
      cy.edges().removeClass('search-flash');
      if (isCancelled) return;

      cy.fit(undefined, 50);
      addToast("Investigation complete — 3 suspicious patterns found", "success");
  ...
  }, [autoInvestigateState, setSelectedAccountId, addToast, setAutoInvestigateState]);
